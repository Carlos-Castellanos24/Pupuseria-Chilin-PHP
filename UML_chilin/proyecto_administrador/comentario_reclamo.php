<?php 	
	...
	$statement = $conexion->prepare('SELECT * FROM vw_comentario WHERE tipo_cometario="R" ');
	$statement->execute();
...
<div class="contenido">
	<br>
	<center><h1>COMENTARIOS RESEÑA</h1></center>
	<br>
	<table>
		<thead>
			<tr>
		...
		<tbody>
			<?php foreach ($resultado as $comentario): ?>
				<tr>
					...
					<td><?php if ($comentario['estado_comentario']=='A'): ?>
							<?= "No leído" ?>
						<?php else: ?>
							<?= "Leído" ?>
						<?php endif ?></td>
